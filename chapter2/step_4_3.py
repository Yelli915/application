...
if __name__ == "__main__" : 
    ...
    ax.pie(values,textprops=dict(color="white",size=20),
       startangle=90,autopct=lambda pct : custom_autopct(pct,sum(values)),)
    ax.legend(labels,loc="center left",bbox_to_anchor=(1,0.5))
    ax.set_title(f"1분기에는 {labels.iloc[0]}에 {values.iloc[0]:,}원을 사용했습니다")
